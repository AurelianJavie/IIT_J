...
# Load sentiment model (Hugging Face default: DistilBERT fine-tuned on SST-2)
print("Loading sentiment analysis model...")
sentiment_analyzer = pipeline("sentiment-analysis")

# Apply to reviews (truncate to first 512 chars to avoid model limits)
df['sentiment'] = df['clean_review'].apply(lambda x: sentiment_analyzer(x[:512])[0]['label'])
df['sentiment_score'] = df['clean_review'].apply(lambda x: sentiment_analyzer(x[:512])[0]['score'])

print("Sample with sentiment analysis:")
print(df[['clean_review','sentiment','sentiment_score']].head())
...
plt.figure(figsize=(6,4))
sns.histplot(df['n_words'], bins=30, kde=True, color="blue")
plt.title("Distribution of Review Lengths")
plt.xlabel("Number of Words")
plt.ylabel("Frequency")
plt.show()

# 3. Sentiment by product (if product_id exists)
if 'product_id' in df.columns:
    plt.figure(figsize=(8,5))
    sns.countplot(x="product_id", hue="sentiment", data=df, palette="Set2")
    plt.title("Sentiment by Product")
    plt.xticks(rotation=45)
    plt.ylabel("Number of Reviews")
    plt.show()

# 4. Average sentiment score by product
if 'product_id' in df.columns:
    plt.figure(figsize=(8,5))
    avg_scores = df.groupby('product_id')['sentiment_score'].mean().reset_index()
    sns.barplot(x="product_id", y="sentiment_score", data=avg_scores, palette="mako")
...
plt.savefig(os.path.join(out_dir, "sentiment_distribution.pdf"))
plt.close()
...
plt.savefig(os.path.join(out_dir, "review_length_distribution.pdf"))
plt.close()
...
if 'product_id' in df.columns:
    ...
    plt.xticks(rotation=45)
    plt.ylabel("Number of Reviews")
    plt.savefig(os.path.join(out_dir, "sentiment_by_product.png"))
    plt.savefig(os.path.join(out_dir, "sentiment_by_product.pdf"))
    plt.close()
...
if 'product_id' in df.columns:
    ...
    plt.xticks(rotation=45)
    plt.ylabel("Avg Sentiment Score (0-1)")
    plt.savefig(os.path.join(out_dir, "avg_sentiment_score_by_product.png"))
    plt.savefig(os.path.join(out_dir, "avg_sentiment_score_by_product.pdf"))
    plt.close()
